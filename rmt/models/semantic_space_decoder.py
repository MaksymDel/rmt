...
class SemanticSpaceDecoder(Model):
    """
    Takes encoded sequence in some semantic vector space (i.e. BERT features) and generates actual
    sentence from it.
    ...
    def __init__(self,
        ...
        super(SemanticSpaceDecoder, self).__init__(vocab)
        self._target_namespace = target_namespace
        self._scheduled_sampling_ratio = scheduled_sampling_ratio

        self._encoder = encoder
        # We need the start symbol to provide as the input at the first timestep of decoding if 
        # there is no teacher forcing, and
    ...
    def forward(self,  # type: ignore
        ...
        Parameters
        ----------
        state : ``Dict[str, torch.LongTensor]``
           The sequence of vectors in semantic space (e.g. produced by bert).
        ...
        # prepare vectors for decoding
        bs = target_vectors.size()[0]
        max_seq_len = target_vectors.size()[1]

        # we intentionally do not target_tokens to obtain mask here, since target tokens are not
        # available during test time; we want our model to learn to igonre padding 
        tgt_mask = target_vectors.new_ones((bs, max_seq_len))

        state = self._init_encoded_state(target_vectors, tgt_mask)
    ...
    def _init_encoded_state(self, embedded_input: torch.Tensor, source_mask: torch.LongTensor) -> Dict[str, torch.Tensor]:
        ...
        self._encoded_bos_symbol = state["encoded_bos_symbol"]
